...
if flag==1
   ...
   ind1n      =  find(W~=0) ;
   ind2n      =  find(H~=0) ;
%    W( ind1 ) =  max(vec(W(ind1n))) * 0.02   ; 
%    H( ind2 ) =  max(vec(H(ind2n))) * 0.02   ;
   W( ind1 ) =  min(vec(W(ind1n))) * 1.0   ; 
   H( ind2 ) =  min(vec(H(ind2n))) * 2.0   ;
   H =  H - min(vec(H(ind2n))) * 2.0   ;
...
%    average   =  0.015       ;
   average   =  mean(A(:))       ;
   W( ind1 ) =  (average*rand(n1,1)./1)  ; 
   H( ind2 ) =  (average*rand(n2,1)./1)  ;   
end
