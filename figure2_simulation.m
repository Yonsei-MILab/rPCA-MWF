...
% min || X - L1 - L2 - S ||2 + ||L1||* + ||L2||* + ||S||1
% im_rpca   mGRE images        (input.im) 
% L1        mGRE images        (input.im) 
% L2        mGRE images        (input.im) 
% S         mGRE images        (input.im) 

for nn = 6:6    
im_rpca = abs(sig_rpca(:,:,:,1:1:32,nn));
te_rpca2 = te_rpca(:,:,:,1:1:32,nn)/1000;
...
msk = abs(im(:,:,:,1)); msk(msk<0.1*max(msk(:))) = 0; msk(msk~=0) = 1;
...
for iz = 1 : floor(N(3)/nlr(3))
    ...
    for ix = 1 : floor(N(1)/nlr(1))
        ...
        for iy = 1 : floor(N(2)/nlr(2))
            ...
            greC = im_rpca(xp,yp,zp,:).*cmp(xp,yp,zp,:);
...
for t = 1:5        
    ...
    if t ~=0
        X_prev = X;
        X(:,:,:,:,1) = ((im_rpca - X_prev(:,:,:,:,2) - X_prev(:,:,:,:,3))...
            - U(:,:,:,:,1) + delta * Z(:,:,:,:,1))/ (1 + delta);
        X(:,:,:,:,2) = ((im_rpca - X(:,:,:,:,1) - X_prev(:,:,:,:,3))...
...
[temp_rpca] = mwi_rpca_pw_v2(input);
temp_rpca = real(temp_rpca);
temp_rpca2(:,:,:,:,:,nn) = temp_rpca;
end
temp_rpca2(temp_rpca2<0) = 0;
temp_rpca2(isnan(temp_rpca2)) = 0;
temp_rpca2(isinf(temp_rpca2)) = 0;
...
zsl = 5;
AWrpca = squeeze((temp_rpca2(:,:,zsl,1,1,:)));
MWrpca = squeeze((temp_rpca2(:,:,zsl,1,2,:)));
Srpca = squeeze((temp_rpca2(:,:,zsl,1,3,:)));
MWFrpca = MWrpca./(AWrpca+MWrpca);
